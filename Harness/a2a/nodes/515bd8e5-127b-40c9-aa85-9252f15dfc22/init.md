...
- Session: 515bd8e5-127b-40c9-aa85-9252f15dfc22
- Runtime: codex
- Agent kind: main
- Role: Main Agent
- Display name: Audit-Codex-Main
- Role title: ceo
- Responsibility: Harness terminal agent
- Capabilities: terminal
- Role profile: Harness/a2a/agent-roles/session-515bd8e5-127b-40c9-aa85-9252f15dfc22.md — read this file; it is your identity and mandate.
- Workflow mode: /wf-max
- Objective: Harness terminal agent
- Prompt: none
- Config revision: 0
...
- Node home: Harness/a2a/nodes/515bd8e5-127b-40c9-aa85-9252f15dfc22
- Env node init: HARNESS_NODE_INIT=
- Env session id: HARNESS_PEER_SESSION_ID=515bd8e5-127b-40c9-aa85-9252f15dfc22
- Env graph node id: HARNESS_WORKFLOW_NODE_ID=session-515bd8e5-127b-40c9-aa85-9252f15dfc22
- Env subagent mode: HARNESS_SUBAGENT_MODE=built-in-subagents
...
- Use Codex native subagent/tool/role path (codex_implement, codex exec, or native agent mechanism)
- Do NOT create WF canvas Agent nodes for workers
- Record fanoutAttempted, channel, roles, and return evidence
- IMPORTANT: If the current Codex runtime does not support native subagents, record clear degradation evidence (runtime version, capability check result, error message) and inform the user. Do not silently skip or pretend native subagents were used.
...
- Main Agent should answer wf-bridge messages with `node Harness/scripts/wf-ui-control.mjs send-agent-message --to <agentNodeIdOrSessionId> --text "..." --project .` so both sides of the bridge are recorded with message envelopes.
